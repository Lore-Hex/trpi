...
`,
	...
	function mockManagedUpdate(targetVersion: string): void {
		vi.stubGlobal(
			...
			vi.fn(async (input: string | URL | Request) => {
				...
				if (url === "https://api.github.com/repos/Lore-Hex/trpi/releases/latest") {
	...
	it("brands update help and keeps the legacy pi target undocumented", async () => {
		const logSpy = vi.spyOn(console, "log").mockImplementation(() => {});
		const errorSpy = vi.spyOn(console, "error").mockImplementation(() => {});
		...
		try {
			...
			expect(stdout).toContain(`${APP_NAME} update ${APP_NAME}`);
			expect(stdout).not.toContain(`${APP_NAME} update pi`);
			expect(errorSpy).not.toHaveBeenCalled();
			expect(process.exitCode).toBeUndefined();
		} finally {
			logSpy.mockRestore();
	...
	it("accepts the branded self-update target and the legacy pi alias", async () => {
		...
		try {
			await expect(runPackageCommandDirectly(["update", APP_NAME])).resolves.toBeUndefined();
	...
	it("updates an installer-managed app through a staged immutable release", async () => {
