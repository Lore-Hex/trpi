 ...
 *
 * Runs TRPI's built-in tools inside a local Gondolin micro-VM. The host working
 ...
 * Usage:
 *   cd /path/to/project
 *   trpi -e /path/to/trpi/packages/coding-agent/examples/extensions/gondolin
 *
 * Requirements:
...
export default function (pi: ExtensionAPI) {
	...
	async function startVm(ctx?: ExtensionContext): Promise<VM> {
		...
		const created = await VM.create({
			sessionLabel: `trpi ${path.basename(localCwd)}`,
			vfs: {
				mounts: {
