 ...
 *
 * Runs TR Confidential Cowork's built-in tools inside a local Gondolin micro-VM. The host working
 ...
 * Usage:
 *   cd /path/to/project
 *   tr-cowork -e /path/to/tr-cowork/packages/coding-agent/examples/extensions/gondolin
 *
 * Requirements:
...
export default function (pi: ExtensionAPI) {
	...
	async function startVm(ctx?: ExtensionContext): Promise<VM> {
		...
		const created = await VM.create({
			sessionLabel: `tr-cowork ${path.basename(localCwd)}`,
			vfs: {
				mounts: {
