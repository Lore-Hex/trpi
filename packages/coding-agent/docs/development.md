...
`PI_SERVER_DIR` overrides the server profile and socket directory (default: `~/.tr-confidential-cowork/server`). `PI_SERVER_ID` selects the logical server ID when `--server-id` is omitted.
