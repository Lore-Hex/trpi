...
```
~/.trpi/agent/sessions/--<path>--/<timestamp>_<uuid>.jsonl
```
...
Sessions can be removed by deleting their `.jsonl` files under `~/.trpi/agent/sessions/`.

TRPI also supports deleting sessions interactively from `/resume` (select a session and press `Ctrl+D`, then confirm). When available, TRPI uses the `trash` CLI to avoid permanent deletion.
...
For TypeScript definitions, inspect `packages/coding-agent/src/` and `packages/ai/src/` in the TRPI source workspace.
...
The exported pi-ai `StopReason` type also includes `"pending"`, but that value is reserved for partial messages in streaming events. Terminal `done`/`error` messages replace it with a completion reason before TRPI persists the assistant message, so `"pending"` should never appear in session JSONL.
...
- `fromHook`: `true` if generated by an extension, `false`/`undefined` if TRPI-generated (legacy field name)
