...
```
~/.tr-confidential-cowork/agent/sessions/--<path>--/<timestamp>_<uuid>.jsonl
```
...
Sessions can be removed by deleting their `.jsonl` files under `~/.tr-confidential-cowork/agent/sessions/`.

TR Confidential Cowork also supports deleting sessions interactively from `/resume` (select a session and press `Ctrl+D`, then confirm). When available, TR Confidential Cowork uses the `trash` CLI to avoid permanent deletion.
...
For TypeScript definitions, inspect `packages/coding-agent/src/` and `packages/ai/src/` in the TR Confidential Cowork source workspace.
...
The exported pi-ai `StopReason` type also includes `"pending"`, but that value is reserved for partial messages in streaming events. Terminal `done`/`error` messages replace it with a completion reason before TR Confidential Cowork persists the assistant message, so `"pending"` should never appear in session JSONL.
...
- `fromHook`: `true` if generated by an extension, `false`/`undefined` if TR Confidential Cowork-generated (legacy field name)
