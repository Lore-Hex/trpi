...
|----------|----------------------|------------------|
| TrustedRouter | `TRUSTEDROUTER_API_KEY` or `TR_API_KEY` | `trustedrouter` |
...
The canonical implementation is [`packages/ai/src/env-api-keys.ts`](../../ai/src/env-api-keys.ts) in this repository; upstream fallback-provider behavior remains compatible with Pi.
...
```json
{
  "trustedrouter": { "type": "api_key", "key": "$TRUSTEDROUTER_API_KEY" },
