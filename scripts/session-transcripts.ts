 ...
 *   --analyze      Spawn tr-cowork subagents to analyze each transcript file
...
function runSubagent(prompt: string, cwd: string): Promise<{ success: boolean }> {
	return new Promise((resolve) => {
		const child = spawn("tr-cowork", ["--mode", "json", "--tools", "read,write", "-p", prompt], {
			cwd,
		...
		child.on("error", (err) => {
			console.error(chalk.red(`  Failed to spawn tr-cowork: ${err.message}`));
...
async function main() {
	...
	const sessionsBase = join(homedir(), ".tr-confidential-cowork/agent/sessions");
	...
	if (!analyzeFlag) {
		console.log("\nRun with --analyze to spawn tr-cowork subagents for pattern analysis.");
		return;
	}

	// Find AGENTS.md files to compare against
	const globalAgentsMd = join(homedir(), ".tr-confidential-cowork/agent/AGENTS.md");
