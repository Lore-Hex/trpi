...
const installPackageName = "tr-confidential-cowork-install";
...
function getInternalWorkspaces(lockPackages, additionalWorkspaceNames = new Set()) {
	...
	for (const [lockPath, entry] of Object.entries(lockPackages)) {
		...
		if (!entry.name.startsWith(internalPackagePrefix) && !additionalWorkspaceNames.has(entry.name) && !internalPackageNames.has(entry.name)) {
			continue;
		}
...
function createInstallerPackageJson(codingAgentPackage) {
	const packageJson = {
		...
		private: true,
		description: "Lockfile root used by the TR Confidential Cowork installer and updater.",
		dependencies: {
...
function generateInstallLock() {
	...
	const internalWorkspaces = getInternalWorkspaces(lockPackages, new Set([codingAgentPackage.name]));
