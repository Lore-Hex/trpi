...
TRPI is a local coding agent that runs inside the security boundary of the user
...
TRPI treats the local user account and files writable by that account as inside
the same trust boundary as the TRPI process itself. If an attacker can modify
files under the user's home directory, workspace, shell startup files,
environment, or TRPI configuration, they can generally influence TRPI and other
local developer tools. Reports that depend on such prior local write access are
not security vulnerabilities unless they demonstrate how TRPI grants that write
...
TRPI inside trusted repositories. Files such as `AGENTS.md` and instructions in
source comments can prompt-inject a coding agent; TRPI does not claim to prevent
prompt injection from trusted local inputs.

## Reporting a Vulnerability

If you believe you found a security vulnerability in TRPI or another package in
...
- Local code execution or sandboxing behavior (TRPI intentionally does not
...
- Public internet exposure of a TRPI installation
...
- Reports requiring the ability to create, modify, delete, or replace files,
  ...
  user-controlled local state on the target machine. This includes `~/.trpi`,
  `~/.trpi/agent/models.json`, workspace files, `AGENTS.md`, skills, extensions,
  extension configuration, dotfiles, and files synchronized through NFS, roaming
  profiles, or dotfile managers, unless the report shows how TRPI itself grants
  that access.
...
For example, a report showing that malicious contents written to a trusted TRPI
configuration file cause TRPI to execute commands, load attacker-controlled tools,
...
affected and that the issue is reachable through TRPI. For exposed-secret reports,
